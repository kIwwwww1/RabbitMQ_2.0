...
from advanced.advanced_connection import (
    MQ_EMAIL_NAME_NEWSLETTER_EMAIL_UPDATES,
    send_log,
)
...
def process_new_message(
    ...
    logger.info(f"[ ] Обновили email пользваотеля для новостей {body.decode()}")

    time.sleep(1)

    end = time.time() - start

    logger.info(f"[X] Обновление email завершено {body.decode()} за {end:05f}")
...
def main():
    with EmailUpdatesRabbit() as rabbit:
        rabbit.consume_messages(
            ...
            queue_name=MQ_EMAIL_NAME_NEWSLETTER_EMAIL_UPDATES,
        )
